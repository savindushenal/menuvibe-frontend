            ...
            <div className="overflow-y-auto overscroll-contain max-h-[calc(92vh-60px)] pb-40 lg:pb-0 lg:max-h-none lg:overflow-visible">
              ...
              <div className="lg:flex lg:flex-row">
                ...
                <div className="p-5 lg:pb-6 lg:overflow-y-auto lg:max-h-[500px] lg:w-1/2 lg:flex lg:flex-col">
                  {/* Title */}
                  <h2 className="text-xl lg:text-3xl font-bold text-barista-dark">{item.name}</h2>
                  ...
                  <div className="mt-4">
                    ...
                    <p className="text-2xl lg:text-3xl font-bold text-barista-orange">
                          ...
                          <button
                            ...
                            className={`w-full flex items-center justify-between p-3.5 rounded-2xl border-2 transition-all ${
                              selectedCustomizations.includes(custom.name)
                                ? 'border-barista-orange bg-gradient-to-r from-barista-orange/5 to-orange-50'
                                : 'border-gray-100 bg-gray-50/50 hover:border-gray-200 hover:bg-gray-50'
                            }`}
                          >
                            <span className={`font-medium text-sm ${
                              selectedCustomizations.includes(custom.name) ? 'text-barista-dark' : 'text-gray-600'
                            }`}>{custom.name}</span>
                            <div className="flex items-center gap-3">
                              <span className={`text-sm font-medium ${
                                selectedCustomizations.includes(custom.name) ? 'text-barista-orange' : 'text-gray-400'
                              }`}>+LKR {custom.price}</span>
                              <div className={`w-6 h-6 rounded-full flex items-center justify-center transition-all ${
                                selectedCustomizations.includes(custom.name)
                                  ? 'bg-barista-orange shadow-lg shadow-orange-500/30'
                  ...
                  <div className="hidden lg:flex mt-6 items-center justify-between p-4 bg-gray-50 rounded-2xl">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-bold text-xl w-8 text-center text-barista-dark">{quantity}</span>
                      <button
              ...
              <div className="p-4 pb-6">
                ...
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex items-center gap-1 bg-gray-100 rounded-full p-1">
                    ...
                    <span className="font-bold text-lg w-8 text-center text-barista-dark">{quantity}</span>
                    <button
                      onClick={() => setQuantity(quantity + 1)}
                      className="w-9 h-9 rounded-full bg-white shadow-sm flex items-center justify-center active:scale-95 transition-transform"
                    >
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-xl font-bold text-barista-dark">LKR {totalPrice.toLocaleString()}</p>
                  </div>
                </div>
                ...
                <motion.button
                  ...
                  className={`w-full py-4 rounded-2xl font-semibold text-white text-base transition-all shadow-lg ${
                    isAdded 
                      ...
                      : 'bg-gradient-to-r from-barista-orange to-orange-500 shadow-orange-500/30 active:scale-[0.98]'
                  }`}
                  whileTap={{ scale: 0.98 }}
                >
                  <AnimatePresence mode="wait">
